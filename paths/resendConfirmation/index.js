        ...
        if (!user){
            ...
            token.save(err => {
                ...
                sgMail.send(mailOptions).then((res)=>{
                    ...
                    //     return res.status(500).send({msg:'Technical Issue!, Please click on resend for verify your Email.'});
                    //  }
                return res.status(200).send('A verification email has been sent to ' + user.emailAdress + '. It will be expire after one day. If you not get verification Email click on resend token.');
            });
        };
    })
}